       ...
       $count=0;
		...
		for($me=0;$me<$j;$me++){
			...
			if ($qty[$me]>0) {
				...
				$mydata = array('facility_code' => $facilityCode,	'kemsa_code' => $ids[$me], 'batch_no' => $batchN[$me] ,
	...
	public function Insert()
	{
       ...
       $count=0;
		...
		for($me=0;$me<$j;$me++){
			...
			$inserttransaction->execute("UPDATE `facility_transaction_table` SET total_issues = (SELECT SUM(qty_issued) 
			FROM facility_issues WHERE kemsa_code = '$ids[$me]' and availability='1' and facility_code='$facilityCode' not like '%Donation%')
                                          WHERE `kemsa_code`= '$ids[$me]' and availability='1' and facility_code='$facilityCode'; ");
			//echo "$numrows records updated";
       ...
       $usernow=$this -> session -> userdata('identity'); 
		...
		for($me=0;$me<$j;$me++){
			...
			if ($qty[$me]>0) {
				...
				$mydata = array('facility_code' => $facilityCode,'kemsa_code' => $ids[$me], 'batch_no' => $batchN[$me] ,'expiry_date' => date('y-m-d',strtotime($Expiry[$me])) ,'qty_issued'=> $qty[$me] ,'balanceAsof'=>$Available[$me],
...
public function donation()
	{
		...
		for($i=0;$i<=$count;$i++){
			...
			//adding data to issues table
				...
				$mydata = array('facility_code' => $facility_c,'kemsa_code' => $kemsa_code[$i], 
